...
mod tests {
    ...
    #[test]
    fn pads_training_sequences_on_the_left() {
        let mut destination = Vec::new();
        append_padded_sequence(&mut destination, &[10, 11]);

        assert_eq!(destination.len(), MAX_SEQ_LEN + 1);
        assert!(
            destination[..MAX_SEQ_LEN - 1]
                .iter()
                .all(|token| *token == PAD_TOKEN)
        );
        assert_eq!(&destination[MAX_SEQ_LEN - 1..], &[10, 11]);
    }

    #[test]
    fn creates_aligned_input_and_target_tensors() {
        let device = Default::default();
        let items = vec![
            "A sufficiently long sentence for tokenization."
                .chars()
                .collect(),
        ];

        let batch: BatchItem<TestBackend> = GenBatcher.batch(items, &device);
        assert_eq!(batch.input.dims(), [1, MAX_SEQ_LEN]);
        assert_eq!(batch.target.dims(), [1, MAX_SEQ_LEN]);

        let input = batch.input.into_data().to_vec::<i32>().unwrap();
        let target = batch.target.into_data().to_vec::<i32>().unwrap();
        assert_eq!(&input[1..], &target[..MAX_SEQ_LEN - 1]);
    }
}
